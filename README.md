...
Write a Flink Job (in Java or the Python PyFlink API) that consumes from your Kafka topic from Phase 2.
...
Kafka: Ingest a stream of text (e.g., from Reddit posts, news headlines).

Flink: Process the stream. Use a pre-trained model (from Hugging Face) within a Flink job to perform sentiment scoring. Enrich the data with user info from a static dataset in Delta Lake (via a lookup).
...
Orchestration: Use Airflow or Prefect to run a nightly batch job that retrains the model on the new data in Delta Lake.
